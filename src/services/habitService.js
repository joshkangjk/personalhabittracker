...
function resolveLegacyGoals(rawGoals, legacyDaily, legacyPeriodRaw) {
  let goals = normalizeGoals(rawGoals);
  const hasAny = goals.daily || goals.weekly || goals.monthly || goals.yearly;
  
  const legacyGoalAmount = Number(legacyDaily ?? 0);
  const period = ["daily", "weekly", "monthly", "yearly"].includes(legacyPeriodRaw) ? legacyPeriodRaw : "daily";

  if (!hasAny && legacyGoalAmount > 0) {
    goals = { ...goals, [period]: clampNumber(legacyGoalAmount) };
  }
  return goals;
}
...
export function habitFromRow(r) {
  return {
    ...
    goals: resolveLegacyGoals(r.goals, r.goal_daily, r.goal_period),
...
export function normalizePublicHabit(h) {
  ...
  return {
    ...h,
    goals: resolveLegacyGoals(h.goals, h.goal_daily ?? h.goalDaily, h.goal_period ?? h.goalPeriod),
